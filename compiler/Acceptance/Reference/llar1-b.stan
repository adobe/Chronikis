...
data {
  ...
  real mu_;
  real<lower = 0.0> sigma_total_;
  real<lower = 0.0> sigma_h_scale_;
}
transformed data {
  matrix[1, N] yy = to_matrix(y');
  real sigmasqr_total_ = square(sigma_total_);
  vector[2] Z_0_ = to_vector({1.0, 1.0});
  vector[2] a0_0_ = to_vector({0.0, mu_});
}
parameters {
  real<lower = 0.0> raw_0_;
  real<lower = 0.0, upper = 1.0> raw_1_;
  real<lower = 0.0, upper = 1.0> rho_;
}
transformed parameters {
  real sigma_h_ = raw_0_ * sigma_h_scale_;
  real sigmasqr_a_ = raw_1_ * sigmasqr_total_;
}
model {
  real sigma_a_ = sqrt(sigmasqr_a_);
  real sigmamu_a_ = sqrt(sigmasqr_total_ - sigmasqr_a_);
  real phi_ = sqrt(1.0 - square(rho_));
  real H_0_ = square(sigma_h_);
  matrix[2, 2] T_0_ = diag_matrix(to_vector({phi_, 1.0}));
  matrix[2, 2] Q_0_ = diag_matrix(to_vector({square(rho_ * sigma_a_), 0.0}));
  matrix[2, 2] P0_0_ = diag_matrix(to_vector({square(sigma_a_), square(sigmamu_a_)}));
  raw_0_ ~ normal(0.0, 1.0) T[0.0, ];
  raw_1_ ~ uniform(0.0, 1.0);
