...
data NumVal = IVal Integer | RVal Scientific
  deriving (Show, Eq)
...
substituteConstQpParams :: ExprT -> ExprT
substituteConstQpParams e = sc Map.empty e
  where
    sc :: (Map VarName NumVal) -> ExprT -> ExprT
    sc _ e'@Var{} = e'
    sc _ e'@Lit{} = e'
    sc cmap (Let typ v edef ebody) =
      Let typ v edef' ebody'
      where
        edef' = sc cmap edef
        ebody' = sc cmap' ebody
        cmap' = case foldConst cmap edef' of
                  Just nval -> Map.insert v nval cmap
                  Nothing -> cmap
    sc cmap (Draw typ v edistr ebody) =
      Draw typ v edistr' ebody'
      where edistr' = sc cmap edistr
            ebody' = sc (Map.delete v cmap) ebody
    sc cmap (Apply TSDType "qp" args) =
      Apply TSDType "qp" $ map fc args
      where
        fc e' = dflt e' $ toLit <$> foldConst cmap e'
    sc cmap (Apply typ fct args) =
      Apply typ fct $ map (sc cmap) args

foldConst :: (Map VarName NumVal) -> ExprT -> Maybe NumVal
foldConst cmap (Var _ v) =
  Map.lookup v cmap
foldConst _ (Lit _ (IntVal n)) =
  Just $ IVal n
foldConst _ (Lit _ (RealVal x)) =
  Just $ RVal x
foldConst _ Draw{} =
  Nothing
foldConst cmap (Let _ v edef ebody) =
  do
    nval <- foldConst cmap edef
    foldConst (Map.insert v nval cmap) ebody
foldConst cmap (Apply typ fct args) =
  mapM (foldConst cmap) args >>= foldConst0 typ fct

foldConst0 :: Type -> FctName -> [NumVal] -> Maybe NumVal
foldConst0 IType fctName args = do
  fct <- Map.lookup fctName foldMapI
  IVal <$> fct args
foldConst0 RType fctName args = do
  fct <- Map.lookup fctName foldMapR
  RVal <$> fct args
foldConst0 _ _ _ = Nothing

foldMapI :: Map FctName ([NumVal] -> Maybe Integer)
foldMapI = Map.fromList
  [ "*" .-> ifct (Just . product)
  , "div" .-> ifct $ fromBinop div
  , "+" .-> ifct (Just . sum)
  , "-" .-> ifct $ fromBinop (-)
  , "negate" .-> ifct $ fromUnop negate
  ]
  where
    ifct f = mapM extractInt >=> f

foldMapR :: Map FctName ([NumVal] -> Maybe Scientific)
foldMapR = Map.fromList
  [ "*" .-> rfct (Just . product)
  -- No "/", because some results cannot be represented exactly as
  -- Scientific values
  , "+" .-> rfct (Just . sum)
  , "-" .-> rfct $ fromBinop (-)
  , "negate" .-> rfct $ fromUnop negate
  , "i2R" .-> mapM extractInt >=> fromUnop fromInteger
  , "square" .-> rfct $ fromUnop square
  , "^" .-> power
  ]
  where
    square x = x * x
    rfct f = mapM extractReal >=> f
    extractReal (RVal x) = Just x
    extractReal  _       = Nothing
    power [RVal x, IVal n] = pow x n
    power _ = Nothing
    pow _ 0 = Just 1
    pow x 1 = Just x
    pow x 2 = Just $ x * x
    pow x 3 = Just $ x * x * x
    pow _ _ = Nothing
    -- No negative exponents, because of possibility that result cannot
    -- be represented exactly as a Scientific value.

extractInt :: NumVal -> Maybe Integer
extractInt (IVal n) = Just n
extractInt _        = Nothing

fromBinop :: (a -> a -> b) -> ([a] -> Maybe b)
fromBinop f = f' where f' [x, y] = Just $ f x y
                       f' _      = Nothing

fromUnop :: (a -> b) -> ([a] -> Maybe b)
fromUnop f = f'  where f' [x]    = Just $ f x
                       f' _      = Nothing
...
removeUnusedDefs :: ExprT -> ExprT
removeUnusedDefs expr = fst $ rud expr
  where
  rud e@(Var _ v) = (e, Set.singleton v) 
  rud e@Lit{} = (e, Set.empty)
  rud (Let typ v def body) =
    let (def', defvars) = rud def
        (body', bodyvars) = rud body
    in
      if Set.member v bodyvars
        then (Let typ v def' body',
              Set.delete v $ defvars `Set.union` bodyvars)
        else (body', bodyvars)
  rud (Draw typ v distr body) =
    let (distr', distrvars) = rud distr
        (body', bodyvars) = rud body
    in
      if Set.member v bodyvars
        then (Draw typ v distr' body',
              Set.delete v $ distrvars `Set.union` bodyvars)
        else (body', bodyvars)
  rud (Apply typ fct args) =
    let mappedargs = map rud args
        args' = map fst mappedargs
        argsvars = Set.unions $ map snd mappedargs
    in
      (Apply typ fct args', argsvars)
